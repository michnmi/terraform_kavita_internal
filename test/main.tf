...
provider "libvirt" {
  ...
  uri   = "qemu+ssh://michnmi@vmhost01/system?keyfile=/home/michnmi/.ssh/id_ecdsa_docker"
}
...
resource "libvirt_volume" "kavita" {
  provider         = libvirt.vmhost01
  name             = "kavita-${var.env}.qcow2"
  pool             = var.env
  base_volume_name = "kavita-base.qcow2"
  format           = "qcow2"
  base_volume_pool = var.env
}

resource "libvirt_domain" "kavita" {
  provider  = libvirt.vmhost01
  name      = "kavita-${var.env}"
  memory    = "1024"
  vcpu      = 2
  ...
  network_interface {
    macvtap  = "enp0s25"
    mac      = "52:54:00:EA:17:83"
    hostname = "kavita-${var.env}"
  }
  ...
  disk {
    volume_id = libvirt_volume.kavita.id
  }
